...
{
    public User(Guid id, string email, string fullname, string pic, string password, string role, string securityStamp,
        int accessFailedCount, DateTime lockoutEnd, DateTime createdAt,
        IEnumerable<string> permissions = null)
    {
        if (string.IsNullOrWhiteSpace(email)) throw new InvalidEmailException(email);

        if (string.IsNullOrWhiteSpace(fullname)) throw new InvalidFullnameException(fullname);

        if (string.IsNullOrWhiteSpace(password)) throw new InvalidPasswordException();

        if (!Entities.Role.IsValid(role)) throw new InvalidRoleException(role);
    ...
    public string NORMALIZED_FULLNAME { get; }
