...
public class GenerateNewTokenHandler : ICommandHandler<GenerateNewToken>
{
    ...
    {
        if (command.Expiration == 0) throw new ArgumentNullException(nameof(command.Expiration));
        if (command.Scopes == null) throw new ArgumentNullException(nameof(command.Scopes));
        ...
        var userClaims = _appContext.Identity.Claims.Where(c => c.Key != JwtClaimTypes.Scope)
            .Select(s => new Claim(s.Key, s.Value.FirstOrDefault() ?? string.Empty));

        var token = new Token
        {
            Audiences = new[] { "my_api" },
            ClientId = clientId,
